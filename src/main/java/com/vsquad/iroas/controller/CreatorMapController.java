...
public class CreatorMapController {
    ...
    @Operation(summary = "크리에이터 맵 추가", description = "크리에이터 맵을 추가합니다.", responses = {
            ...
            @ApiResponse(responseCode = "400", description = "맵 추가 실패", content = @Content(schema = @Schema(name = "맵 추가 실패", example = "에러 메시지"), mediaType = "application/json"))
    })
    ...
    @Operation(summary = "크리에이터 맵 조회", description = "크리에이터 맵을 조회합니다.", responses = {
            ...
            @ApiResponse(responseCode = "400", description = "맵 조회 실패", content = @Content(schema = @Schema(name = "맵 조회 실패", example = "에러 메시지"), mediaType = "application/json"))
    })
    ...
    @Operation(summary = "크리에이터 맵 목록 조회", description = "정렬 값, 정렬 순서, 페이지 당 요소 수, 페이지 번호를 입력, 조건에 맞는 요소 목록 조회", responses = {
            @ApiResponse(responseCode = "200", description = "맵 목록 조회 성공", content = @Content(schema = @Schema(implementation = Page.class), mediaType = "application/json")),
            @ApiResponse(responseCode = "400", description = "맵 목록 조회 실패", content = @Content(schema = @Schema(name = "맵 목록 조회 실패", example = "에러 메시지"), mediaType = "application/json"))
    })
    @Parameters({
    ...
    @Operation(summary = "크리에이터 맵 제거", description = "맵을 삭제 합니다.", responses = {
            ...
            @ApiResponse(responseCode = "400", description = "맵 제거 실패", content = @Content(schema = @Schema(name = "맵 제거 실패", example = "에러 메시지"), mediaType = "application/json"))
    })
